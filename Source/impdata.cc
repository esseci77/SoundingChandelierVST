// -------------------------------------------------------------------------
//
//    Copyright (C) 2006-2008 Fons Adriaensen <fons@kokkinizita.net>
//    
...
int Impdata::read_ext (uint32_t k, float *p)
{
    ...
    if (_aldfile)
    {
        k = (uint32_t)fread (p, (_type & TYPE__CHAN) * sizeof (float), k, _aldfile);
    }
    else if (_sndfile)
    {
	k = (uint32_t)sf_readf_float (_sndfile, p, k);
    }
    else return ERR_MODE;
    _i_fram += k;
    return k;
...
int Impdata::write_ext (uint32_t k, float *p)
{
    ...
    if (_aldfile)
    {
        k = (uint32_t)fwrite (p, (_type & TYPE__CHAN) * sizeof (float), k, _aldfile);
    }
    else if (_sndfile)
    {
	    k = (uint32_t)sf_writef_float (_sndfile, p, k);
    }
    else return ERR_MODE;
    _i_fram += k;
    return k;
